...
#     print(period_exp)
#     time_insp = np.linspace(0,period_insp,601)
...
#     period_exp = period - period_insp
#     print(period_exp)
#     time_insp = np.linspace(0,period_insp/2,50)
#     time_exp = np.linspace(0,period_exp,100)

#     # Inspiration phase 1
#     Q_insp_1 = -np.cos((np.pi/period_insp)*time_insp)
#     scale_insp_1 = tidal_volume/2 / np.trapz(Q_insp_1,x=time_insp)
#     Q_insp_1 = scale_insp_1 * np.cos((np.pi/period_insp)*time_insp)
#     print("Inspiration tidal volume: ", np.trapz(Q_insp_1, x=time_insp))
...
#     # Inspiration phase 1
#     Q_insp_2 = -np.sin((np.pi/period_insp)*time_insp)
#     scale_insp_2 = tidal_volume/2 / np.trapz(Q_insp_2,x=time_insp)
#     Q_insp_2 = scale_insp_2 * np.sin((np.pi/period_insp)*time_insp)
#     print("Inspiration tidal volume: ", np.trapz(Q_insp_2, x=time_insp))

#     # combine
#     Q = np.concatenate((Q_insp_1,Q_exp,Q_insp_2))
#     time = np.concatenate((time_insp, time_exp + time_insp[-1], time_insp + time_insp[-1] + time_exp[-1]))
...
def breathing_waveform_sinusoid(tidal_volume, period, period_insp, D_inlet, D_min):
    ...
    scale_insp_2 = tidal_volume / np.trapz(Q_insp_2,x=time_insp)
    Q_insp_2 = scale_insp_2 * np.sin((np.pi/(period_insp-0.001))*(time_insp-0.001))
    print("Inspiration tidal volume: ", np.trapz(Q_insp_2, x=time_insp))
    
    # Expiration phase
    Q_exp = np.sin((np.pi/period_exp)*time_exp)
    scale_exp = exp_tidal_volume / np.trapz(Q_exp, x=time_exp)
    Q_exp = scale_exp*np.sin((np.pi/period_exp)*time_exp)
    print("Expiration tidal volume: ", np.trapz(Q_exp, x=time_exp))
    ...
    # combine
    Q = np.concatenate((Q_insp_2,Q_exp))
    time = np.concatenate((time_insp, time_exp + time_insp[-1]))
