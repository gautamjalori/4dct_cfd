...
file_path = 'D:/Jalori/RobinSequence/4dct109/kymograph_chestmotion_109.csv'

# Replace 'column1', 'column2' with the names of the columns you want to read
columns_to_read = ['time', 'smooth chest motion', 'chest motion gradient']
...
c_motion = data['smooth chest motion'].to_numpy()
grad_c_motion = data['chest motion gradient'].to_numpy()
...
dc_motion_smooth_dt = np.gradient(c_motion, time)


# Assuming column1 is the x-axis and column2 is the y-axis
plt.plot(time, c_motion, label='time vs chest motion')
# plt.plot(time, c_motion_smooth, color='r', label='Smoothed Chest Motion')
plt.plot(time, dc_motion_smooth_dt, color='g', label='Grad Smoothed Chest Motion')
plt.plot(time, grad_c_motion, color='b', label='Expected Grad Smoothed Chest Motion')
